...
void processFFT() {
  ...
  if(((smoothedPowers.beta/ T)*100)>15.0)  // Beta percentage above 15
  {
    if(menu) // Menu is active
    {
      for(int i=0; i<4; i++)
      {
      ...
      menu= !menu;  // Disable the menu
      Serial.println(0);
    }
  }
}

void showPixels()  // Turn all LEDs white 
{
  ...
  if(timer <= 0){
    ...
    if (currentEEGEnvelope > BlinkThreshold && (nowMs - lastBlinkTime) >= BLINK_DEBOUNCE_MS) {
      ...
      if (blinkCount == 0) {
        ...
        {
          // Move to next menuIndex when double blink is detected
          if(menuIndex==5)
          {
            menuIndex = 0;
